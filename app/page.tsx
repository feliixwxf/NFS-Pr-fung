  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        {view === "progress" && <ProgressView progress={questionProgress} setView={changeView} />}
...
function shuffledQuestions(previous: SingleChoiceQuestion[] = []) {
  const next = ([1, 2, 3] as const).flatMap((difficulty) => {
    const tier = apoplexSingleChoiceQuestions.filter((question) => question.difficulty === difficulty).map((question) => {
...
function SingleChoiceQuiz({ onProgressChange, standalone = false }: { onProgressChange: (progress: QuestionProgress) => void; standalone?: boolean }) {
...
function QuizTraining({ onProgressChange }: { onProgressChange: (progress: QuestionProgress) => void }) {
  return <section className="quiz-page mc-training-page"><div className="section-heading"><div><span className="eyebrow">MC-Training · Schlaganfall</span><h1>Single-Choice zum Krankheitsbild</h1><p>Vier plausible Antwortmöglichkeiten fordern medizinisches Verständnis und genaues Verknüpfen. Die Schwierigkeit steigt bis zum Staatsexamensniveau; alle Ergebnisse fließen in dieselbe Statistik ein.</p></div><div className="source-badge"><b>18</b><span>Fragen</span></div></div><SingleChoiceQuiz standalone onProgressChange={onProgressChange} /><div className="mc-training-rule"><span>3×</span><p><b>Dreistufiger Lernstand</b>Richtig beantwortete Fragen steigen auf 33 %, 67 % und 100 %. Eine falsche Antwort setzt nur die betreffende Frage auf 0 % zurück.</p></div></section>;
}

function ProgressView({ progress, setView }: { progress: QuestionProgress; setView: (view: View) => void }) {
  const summary = summarizeQuestionProgress(progress);
  const chapterRecords = apoplexSingleChoiceQuestions.map((question) => progress[question.id]);
  const answered = chapterRecords.filter(Boolean);
