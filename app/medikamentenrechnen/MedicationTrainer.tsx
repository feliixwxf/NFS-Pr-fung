...
function Feedback({current,result}:{current:(typeof medicationCases)[number];result:Result}) { const count=current.answers.filter(x=>isCorrect(result.answers[x.id],x.value,x.decimals,x.unit)).length; return <section className={`case-feedback ${count===current.answers.length?"correct":count?"partial":"wrong"}`}><h3>{count===current.answers.length?"Korrekt":count?"Teilweise korrekt":"Noch nicht korrekt"}</h3>{current.answers.map(answer=>{const ok=isCorrect(result.answers[answer.id],answer.value,answer.decimals,answer.unit);return <div key={answer.id}><b>{ok?"✓":"×"} {answer.label}: {result.answers[answer.id]} {answer.unit}</b><p>Exaktes Ergebnis: {String(answer.value).replace(".",",")} {answer.unit}. {answer.calculation}.</p>{ok&&answer.unit==="ml"&&Number(result.answers[answer.id].replace(",","."))!==answer.value&&<small>Als ganze Milliliterangabe akzeptiert; merke dir trotzdem den exakten Wert.</small>}{!ok&&<small>{answer.errorHint||current.hint}</small>}</div>})}<footer><b>Regel</b> {medicationRules.find(r=>r.id===current.ruleId)?.parameter}<br/><b>Quelle</b> {medicationRules.find(r=>r.id===current.ruleId)?.source}</footer></section> }
